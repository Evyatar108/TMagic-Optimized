...
namespace TorannMagic.AutoCast
{
                ...
                if (distanceToTarget > minDistance && caster.CurJob.locomotionUrgency >= LocomotionUrgency.Jog)// && caster.CurJob.bill == null)
                {
                    ...
                    else
                    {
                        ...
                        if (canReach && phaseToCell.IsValid && phaseToCell.InBounds(caster.Map) && phaseToCell.Walkable(caster.Map) && !phaseToCell.Fogged(caster.Map))// && ((phaseToCell - caster.Position).LengthHorizontal < distanceToTarget))
                        {
                            ...
                            float futureCost = ppf.TotalCost;
                            ppf.ReleaseToPool();
                            bool isCloser = currentCost > futureCost;
    ...
    {
        public static void Evaluate(CompAbilityUserMagic casterComp, TMAbilityDef abilitydef, PawnAbility ability, MagicPower power, HashSet<string> validAddictionNames, out bool success)
        {
            success = false;
            if (casterComp.Mana.CurLevel >= casterComp.ActualManaCost(abilitydef) && ability.CooldownTicksLeft <= 0)
            {
